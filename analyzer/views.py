...
def sales_stats(request):
    """API endpoint for sales analysis (distribution and trend)."""
    component_type = request.GET.get('component_type', 'cpu').lower()
    chart_type = request.GET.get('chart_type', 'distribution')  # distribution or trend

    if component_type not in COMPONENT_MODELS:
        return JsonResponse({'error': '无效配件类型'}, status=400)

    cache_key = f'sales_stats_{component_type}_{chart_type}'
    cached_data = cache.get(cache_key)
    if cached_data:
        return JsonResponse(cached_data)
    ...
    try:
        ...
        if chart_type == 'distribution':
            ...
            items = model.objects.filter(comment_count__gt=0).values('comment_count')
            if not items:
                return JsonResponse({'error': '无销售数据'}, status=404)

            sales = np.array([item['comment_count'] for item in items])
            bins = np.histogram_bin_edges(sales, bins=10)
            ...
            data = {
                ...
                'stats_summary': {
                    ...
                    'avg_sales': float(sales.mean()) if sales.size else 0,
                    'min_sales': float(sales.min()) if sales.size else 0,
                    'max_sales': float(sales.max()) if sales.size else 0,
                }
            }

        elif chart_type == 'trend':
            # Average sales trend over 90 days
            if not price_history_model:
                return JsonResponse({'error': '无历史销售数据'}, status=404)

            end_date = datetime.now().date()
            start_date = end_date - timedelta(days=90)
            history_data = price_history_model.objects.filter(
                date__range=[start_date, end_date]
            ).values('date').annotate(
                avg_sales=Avg('comment_count')
            ).order_by('date')
            ...
            data = {
                'sales_trend': [
                    {
                        'date': item['date'].strftime('%Y-%m-%d'),
                        'avg_sales': float(item['avg_sales']) if item['avg_sales'] else 0
                    }
            ...
            return JsonResponse({'error': '无效图表类型'}, status=400)
